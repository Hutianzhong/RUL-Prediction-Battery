function pre_data = LSTM_main(h,train_data,test_data)
%% 预处理
...
for m = train_num + 1:train_num + test_num
    ...
    test_input = [test_input(2:end); test_data(m-train_num)];
    test_input = tramnmx(test_input,min_input,max_input);
end
...
title('LSTM预测')
hold on
plot(1:size([train_data test_data],2),[train_data test_data], '*-', 'color','b', 'linewidth', 1);
plot(size(train_data,2) + h:h:size([train_data test_data],2),pre_data, 'o-','color','r','linewidth', 1);
plot([size(train_data,2) size(train_data,2)],[-0.008 0.002],'g-','LineWidth',4);
legend({ '真实值', '预测值'});
end
