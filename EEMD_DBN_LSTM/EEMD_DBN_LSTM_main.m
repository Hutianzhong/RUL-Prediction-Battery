...
for i = num_train+h:h:size(s,1)
    ans = [ans DBN_main(h,lag,i-h-win+1,i-h-1,after_imf(end,:))];
end
...
title('DBN预测')
hold on
plot(1:size(s,1),after_imf(end,:), '*-', 'color','b', 'linewidth', 1);
plot(num_train+h:h:size(s,1),ans,'o-','color','r','linewidth', 1);
plot([num_train num_train],[0.8 0.9],'g-','LineWidth',4);
legend({ '真实值', '预测值'});
...
for i = 2:dim-1
    ...
    ans = [ans; LSTM_main(h,after_imf(i,1:num_train),after_imf(i,num_train+1:end))];
end
...
title('总预测')
hold on
plot(1:size(s,1),s, '*-', 'color','b', 'linewidth', 1);
plot(num_train+h:h:size(s,1),pre, 'o-','color','r', 'linewidth', 1);
plot([num_train num_train],[0.8 0.9],'g-','LineWidth',4);
legend({ '真实值', '预测值'});
